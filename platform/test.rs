...
#[cfg(target_os="linux")]
mod fragment_tests {
    use platform::{self, OsIpcChannel};
    use std::mem;
    ...
    create_with_n_fds!(full_packet, 0, *FRAGMENT_SIZE);

    create_with_n_fds!(full_packet_with_1_fds, 1, *FRAGMENT_SIZE);
    create_with_n_fds!(full_packet_with_2_fds, 2, *FRAGMENT_SIZE);
    create_with_n_fds!(full_packet_with_3_fds, 3, *FRAGMENT_SIZE);
    create_with_n_fds!(full_packet_with_4_fds, 4, *FRAGMENT_SIZE);
    create_with_n_fds!(full_packet_with_5_fds, 5, *FRAGMENT_SIZE);
    create_with_n_fds!(full_packet_with_6_fds, 6, *FRAGMENT_SIZE);

    // MAX_FDS_IN_CMSG is currently 64.
    create_with_n_fds!(full_packet_with_64_fds, 64, *FRAGMENT_SIZE);

    create_with_n_fds!(overfull_packet, 0, *FRAGMENT_SIZE + 1);

    // In fragmented transfers, one FD is used up for the dedicated channel.
    create_with_n_fds!(overfull_packet_with_63_fds, 63, *FRAGMENT_SIZE + 1);
}
